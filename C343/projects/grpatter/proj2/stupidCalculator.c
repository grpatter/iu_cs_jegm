...
int main(){
	...
	while(1){
		...
		if(*equation == 'q'){//quit
			...
			reverseStack();//flip stack around (structure..)
...
void stack_ops(char *input){
  ...
  if(*input == '+'){
    ...
    var_count++;//adjust vars counter
  }
}

void reverseStack(){
  if(isEmptyStack()){//if empty
    return;//return to caller
  }else{
    int temp = pop();//hold val in temp
    reverseStack();//recur
    push(temp);//readd to stack
  }
}

void print_stack(){//print the stack out
  if(isEmptyStack()){
    return;//get outif we are at the end
  }else{
