...
char *read_send_file(string my_ip){
  ...
  char * buffer;
  size_t result;
  pFile = fopen(send_name.c_str(),"rb");
  if(pFile == NULL){
...
int get_router_id(string cur_ip){
	int ehid = 0;
	vector<Routers>::iterator hi;
	for(hi = r_inst.begin(); hi<r_inst.end(); hi++){
		if(cur_ip == hi->ext_ip){
			ehid = hi->r_id;
			return ehid;
		}
...
int get_router_delay(int my_id, int next_id) {
	int delay = 0;
	//look through router links to match my_id & nextIP
	vector<Links>::iterator ri;
	for(ri = rr_inst.begin(); ri < rr_inst.end(); ri++){
		if(my_id == ri->r1_id && next_id == ri->r2_id){
			delay = ri->delay;
			return delay;
		}
	}
	//look through end host links to match my_id & nextIP
	vector<Ehlinks>::iterator li;
	for(li = rh_inst.begin(); li < rh_inst.end(); li++) {
		if(my_id == li->r_id && next_id == li->eh_id) {
			delay = li->delay;
			return delay;
		}
	}
